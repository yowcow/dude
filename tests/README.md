...
- `git_repo_origin_head <dir> <branch>` — points `refs/remotes/origin/HEAD` at
  ...
  stale or wrong value here is ignored. The target need not exist: `git
  symbolic-ref` accepts a dangling one, which is how a test builds "the
  default branch is named but absent from the remote".
