...
Six, and none of them lowers the bar in **What counts as a finding**. An initial pass runs inline with the whole list, except where the caller declared a dispatch case (a)–(c) or the orchestrator marked (d), which gives the whole list to its single reviewer; a re-run after an Important finding is scoped per **Caller contract**.

- **Evidence sufficiency** — whether each claim's stated evidence actually carries it: a conclusion resting on a sample, a single occurrence, or a correlation presented as though it were the measurement; a number with no command, window, or source behind it. Fact claims (command output, measurements, `path:line`) are met against the trace left plus whether the data fits the claim, never by re-running the measurement; judgment claims (inference beyond the trace) get full re-verification. Absence/exhaustiveness/"only" claims stay under Sweep completeness even when count-backed.
