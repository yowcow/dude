...
- **Evidence sufficiency** — whether each claim names its backing artifact: a `path:line` or a command plus its measured value per claim. Whether the artifact actually carries the claim is checked only in a dispatched pass, except the full-scope inline re-run after Critical handling where neither (a) nor (d) applies, which checks it inline, and the scoped inline re-run after an Important Evidence-sufficiency finding, which checks it inline scoped to the edited scope.
