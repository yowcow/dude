...
Then stop, and name `pr-to-ready` as the next entry **without invoking it**, handing it the PR's URL — that reference is its whole entry, and it sets up its own workspace from it. Which flow runs next is the caller's decision, not this skill's.
