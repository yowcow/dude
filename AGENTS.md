...
`make manifest` runs the official validators below, each of which names the field it rejects. It also JSON-parses `.agents/plugins/marketplace.json`, `package.json`, `hooks/hooks.json`, `.muse-plugin/plugin.json`, and `.muse-plugin/marketplace.json`, which the vendor validators do not all cover:
