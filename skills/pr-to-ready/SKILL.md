...
2. Claude leaves no actionable feedback unaddressed — a "looks good"/LGTM-equivalent comment, or a finding of this round, addressed in 2-3's sense (a human reviewer's comments count the same way);
3. Copilot's latest round leaves no actionable finding unaddressed: `<skill-dir>/scripts/list-unresolved-threads.sh <owner> <repo> <pr-number>` **exits 0 with empty stdout**, and `<skill-dir>/scripts/list-suppressed-comments.sh <owner> <repo> <pr-number>` **exits 0** with every entry it prints a finding of this round, addressed in 2-3's sense. The two sides are read differently because Copilot's findings arrive on two paths and neither script sees the other's: a comment listing carries no resolution state, and a finding Copilot suppressed never became a thread at all, so it can be neither replied to nor resolved and stays in that listing regardless of how it is addressed — **addressed, not absent, is all the suppressed side can be read for** — while a rejected thread is replied to and resolved, and so does drop out of the thread listing. **The review body's own `Comments generated: 0 new` is not evidence here** — that count leaves suppressed findings out, so a review reporting zero new comments can still carry one;
