...
The one form of that ladder is `implement-work/scripts/resolve-default-branch.sh`, called as a subprocess by the three scripts that need it; the `STOP` each of them prints is its own output contract and stays with it.
...
- **Why merge, and not rebase.** Rebase rewrites commits that are already pushed, so it needs a force-push, which is barred. A merge reaches the same state — the base is in — by adding a commit instead of rewriting any, so it needs no rewrite and no exception.
