...
# --- case 23: a symlink at the expected test path is not coverage -----------
# `-f` follows the link, so the coverage guard counted a symlinked test file as
# covered, while run.sh collects with `find -type f` and never collects it: a
# test that is counted and never run. That is this gate's own reason for
# existing (nobody is running the code) reached through its front door.
# Measured on the unfixed gate: `1 script(s), 1 with tests, 0 exempted` and exit
# 0, with `find tests -type f -name '*_test.sh'` finding nothing.
#
# The link target must be a non-empty readable regular file, or the `-f`/`-s`
# guard fails on the target and the unfixed gate already reports the script as
# uncovered — the case would pass for the wrong reason. It sits outside
# `skills/`, so it is not itself enumerated.
#
# The row keys on the message, not only the exit status: reporting this as a
# plain "no test for" would tell an author whose file is right there that it is
# missing.
total=$((total + 1))
fails_here=0
root="$(tree_new)"
...
printf '#!/usr/bin/env bash\nexit 0\n' >"${root}/real_body.sh"
mkdir -p "${root}/tests/alpha"
ln -s "${root}/real_body.sh" "${root}/tests/alpha/a_test.sh"
run_sut bash "$SUT" "$root"
if ! check_eq 'symlinked test: exit' 1 "$SUT_STATUS"; then fails_here=1; fi
if ! check_stderr_has 'symlinked test: names the symlink and why' \
  'expected test for skills/alpha/scripts/a.sh is a symlink, which tests/run.sh does not collect: tests/alpha/a_test.sh'; then fails_here=1; fi
if [ "$fails_here" -ne 0 ]; then failed=$((failed + 1)); fi

# --- case 24: an allowlisted script is exempt even with a symlinked test ----
# The refusal in case 23 must not overtake the allowlist: an exemption says the
# script needs no test at all, so whatever sits at the expected path is beside
# the point. This is what pins the *order* of the two guards — a fix that
# reported the symlink before consulting the allowlist would fail here while
# passing case 23.
total=$((total + 1))
fails_here=0
root="$(tree_new)"
...
AL
run_sut bash "$SUT" "$root"
if ! check_eq 'symlinked test, allowlisted: exit' 0 "$SUT_STATUS"; then fails_here=1; fi
if ! check_bytes 'symlinked test, allowlisted: stdout' \
  'scripts-have-tests: 1 script(s), 0 with tests, 1 exempted by the allowlist\n'; then fails_here=1; fi
if [ "$fails_here" -ne 0 ]; then failed=$((failed + 1)); fi
