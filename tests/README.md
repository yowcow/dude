...
general exemption path. The one named carve-out is `AUTHORING.md`'s single native `gh` operation rule: a call `plan-work` makes directly has no script, so the gate has nothing to count for it.
