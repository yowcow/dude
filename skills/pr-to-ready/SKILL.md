...
- Which of Copilot, Claude, and requesting (`superpowers:requesting-code-review`) should this run request, from none to all three? Default Copilot and Claude; requesting is opt-in. Record the answer as the **reviewers** set. Step 2-1 requests only from that set. Do not probe availability here — that is 2-1, after 2-0.
