...
1. Gather the inputs: the target report, the question it answers, its stated sources, the lenses this pass covers, the edited scope (the whole report on an initial pass; the actual report edits on a re-run), and the record of earlier passes if the caller supplied one.
