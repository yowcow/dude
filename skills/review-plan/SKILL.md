...
This pass is clean when no finding survives step 3. A `needs-user` survives it: the pass ends non-clean, and that finding is reported as blocking — applied by nobody and rejected by nobody, since no edit to the plan settles it — for the caller to put to a person rather than to another pass. Another pass is the obvious alternative and it is the wrong one: with the finding neither accepted nor rejected, **Dispatch**'s revision rule has no lens left to send out, so the re-run comes back clean over a question nobody answered. What the person settles arrives here as an ordinary revision, that finding among the accepted and fixed in the record the caller hands over. `pr-to-ready`'s stop condition 3 is the precedent for a skill stopping itself on this verdict.
