...
- **SessionStart stub sentences are restated across emitters.** The four stub sentences live canonically in `hooks/session-start` and are repeated in `.opencode/plugins/dude.js` (plus a pointer in `skills/using-dude/SKILL.md` and prose in `README.md`). The test in `tests/opencode-plugin_test.sh` pins the repetition under normalization (marker, envelope, and install path stripped before comparison). The (c): a divergent copy silently injects different workflow rules per runtime with no gate catching it.
