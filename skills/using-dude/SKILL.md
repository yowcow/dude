...
- **A deliverable whose substance is findings, a judgment, or a claim of absence goes through `review-findings` before it reaches that record.**
  ...
  - Run one pass, restate flagged claims as unsettled / not measured. The required clean is that the pass finished, not that it returned no findings. The one further invocation permitted is `review-findings`' subtractive verification pass on the ledger's accepted entries (plus intake-routed rejected-with-new-evidence entries); every other second invocation is forbidden. That pass is a loop under **Loop convergence** below, with its round, same-finding, and clean defined in `review-findings`.
