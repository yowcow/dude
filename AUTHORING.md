...
What all of this protects is one thing: that the basic flow — `(investigate-*, settle-question →) plan-work → implement-work → pr-to-ready` — reads fluently enough to run, review gates included. Preparation for rare exceptions is deliberately not built in. The run stops and asks a person instead.
...
1. **No command block in a `SKILL.md`.** This one is structural rather than a matter of will: there is nowhere in a `SKILL.md` for a command to go, so the question becomes where it *does* go — `scripts/` if it is worth keeping, nowhere if the deletion test says (a) or (b). A single native `gh` operation stays unwrapped: where creating or attaching a record is one native `gh` call with no flag-string, parsing, or branching of its own, no script wraps it and `tests/scripts-have-tests.sh` has nothing to count. The (c): routing that call through the raw `sub_issues` endpoint identifies the child by database id rather than by issue number, so a number passed there silently attaches whichever issue holds that id from whatever repository.
...
- **SessionStart stub sentences are restated across emitters.** The three stub sentences live canonically in `hooks/session-start` and are repeated verbatim in `.opencode/plugins/dude.js` (plus a pointer in `skills/using-dude/SKILL.md` and prose in `README.md`). The test in `tests/opencode-plugin_test.sh` pins the repetition byte-for-byte. The (c): a divergent copy silently injects different workflow rules per runtime with no gate catching it.
