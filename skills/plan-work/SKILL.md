...
- **Creating a child and attaching it to its parent is one native `gh` operation**, so no script wraps it, reading its body from a file rather than from a flag string. Never attach through the raw `sub_issues` endpoint — it identifies the child by database id rather than by issue number, so a number passed there attaches whichever issue happens to hold that id, silently and from whatever repository.
