...
- condition 1 → whichever of Step 1's three paths the watch answered on: a failing conclusion is diagnosed and fixed there, borrowing the diagnosis and not Step 1's own loop, so it isn't counted against Step 1's rounds; a watcher-side status is the third terminal state here too;
