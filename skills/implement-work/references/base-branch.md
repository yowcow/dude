...
- **Why merge, and not rebase.** Rebase rewrites commits that are already pushed, so it needs a force-push, which the guidelines ban outright. A merge reaches the same state — the base is in — by adding a commit instead of rewriting any, so it needs no rewrite and no exception.
